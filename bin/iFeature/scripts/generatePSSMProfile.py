...
def generatePSSMProfile(fastas, outDir, blastpgp, db):
	...
	for i in fastas:
		...
		with open(name + '.txt', 'w') as f:
			f.write('>'+name+'\n'+sequence + '\n')
		#######
		# the cmd was modified to fit the new version of blast 2.16.0
		# myCmd = blastpgp + ' -i ' + name + '.txt' + ' -d ' + db + ' -b 0 -v 5000 -j 3 -h 0.001 -Q ' + outDir + '/' + name + '.pssm'
		myCmd = blastpgp + ' -query ' + name + '.txt' + ' -db ' + db + ' -num_alignments 0 -num_descriptions 5000 -num_iterations 3 -inclusion_ethresh 0.001 -out_ascii_pssm ' + outDir + '/' + name +'.pssm'
		#######
